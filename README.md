# 校园汽车租赁管理系统 (Campus Car Rental System)

这是一个面向高校师生的汽车租赁信息平台，专注于周末结伴周边自驾游场景，提供车辆浏览、在线预约、订单管理和信用评价等功能。基于 Django 框架开发，具备完整的车辆管理、用户管理和租赁管理能力。

## 应用场景

本平台专为高校学生群体设计，聚焦于短途周边自驾游需求：

- **周末结伴出游**：3-5 人组队前往周边景点、露营地、特色小镇
- **节假日近郊游**：探访农家乐、徒步登山、户外野餐
- **灵活低门槛**：相比传统商业租车平台，降低押金门槛和租期限制
- **信息化管理**：通过线上平台整合车辆资源，实现透明化预约与结算
...
- ✅ 车辆收藏功能
- ✅ 车辆对比功能
- ✅ **个性化推荐**（基于历史订单智能推荐合适车型）

#### 订单管理
- ✅ 创建租赁订单（选择车辆、日期、门店）
- ✅ **多种押金方式**（现金押金、学生卡抵押、VIP免押金、首次免押金）
- ✅ 订单列表查看（我的订单）
- ✅ 订单详情查看（费用明细、支付记录）
- ✅ 订单取消（自动退款）
- ✅ 订单评价（1-5星评分）
...
- ✅ **业务逻辑**：
  ...
  - 押金自动退还
  - 订单取消自动退款
  - **动态押金计算**：基于车辆价值、租期、信用评分的智能定价
- ✅ **数据验证**：多层次验证（表单验证、模型验证、业务规则验证）
- ✅ **性能优化**：使用 `select_related`、数据库索引、查询优化、缓存机制
...
- ✅ **评价系统**：车辆评价展示、评分分布统计
- ✅ **订单状态跟踪**：完整的状态流转和自动更新
- ✅ **动态押金系统**：智能计算押金，综合考虑车辆价值、租期、信用评分
- ✅ **个性化推荐**：基于用户历史订单和偏好分析，智能推荐合适车辆
- ✅ **数据可视化**：仪表盘图表展示，包括收入趋势、订单分布等
...
### 动态押金计算系统
- **智能定价**：根据车辆价值、租期时长、客户信用评分综合计算押金
- **动态调整**：
  - 车辆价值越高，基础押金越高（车辆价值的5%）
  - 租期越长，时长系数越大（0-30天，每天增加1%）
  - 信用评分越高，押金系数越小（0-100分）
- **特殊政策**：VIP会员享受免押金优惠
- **上下限保护**：最低为3天租金，最高为车辆价值的15%

### 个性化推荐系统
- **多级推荐策略**：
  - **基于历史订单**：分析用户过往租赁记录，推荐相似车型
  - **偏好分析**：智能识别用户喜好的品牌、车型、座位数
  - **热门车型**：根据租赁次数推荐最受欢迎的车辆
  - **新品推荐**：为新用户推荐最新添加的车辆
- **智能缓存机制**：使用缓存提升推荐响应速度，优化用户体验
- **去重逻辑**：自动排除用户已租过的车辆，提供多样化选择
- **实时状态过滤**：只推荐当前可用的车辆，确保推荐准确性

### 数据可视化仪表盘
- **收入趋势图**：近6个月收入趋势折线图，直观展示业务增长情况
- **订单分布图**：订单状态分布饼图，实时掌握运营状态
- **关键指标**：车辆总数、客户总数、订单总数、本月/今日收入等一目了然
- **快速操作**：2x2网格布局，快速访问常用功能
- **最近活动**：实时显示最新5条订单动态

### 学生卡抵押系统
- **零现金压力**：学生无需支付押金，凭学生卡即可租车
- **即时归还**：还车时当场取回学生卡，无需等待退款
- **线下核验**：取车时人工核验学生卡原件，确保安全
- **多重选择**：支持现金押金、学生卡抵押、VIP免押金、首次免押金四种方式
- **灵活切换**：用户可根据自身情况选择最适合的押金方案
